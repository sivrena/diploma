...
def findParameters(particle_list, stepnumber, z):
    ...
    for m in range(k):
        ...
        gx.plot(x, avx, label="Av. Vel_x in layer " + str(round(thick[m], 2)) + "nm")
        gx.legend(loc="upper right")
        gy.plot(x, avy, label="Av. Vel_y in layer " + str(round(thick[m], 2)) + "nm")
        gy.legend(loc="upper right")
        gz.plot(x, avz, label="Av. Vel_z in layer " + str(round(thick[m], 2)) + "nm")
