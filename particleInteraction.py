...
class Particle:
    def __init__(self, mass, radius, epsilon, sigma, position, velocity, force, acceleration, alpha, adsorbate):
        ...
        self.solforce = [np.copy(self.force)]

    def compute_step(self, step, f, z, borders):
        ...
        self.velocity += 1 / 2 * (a + self.acceleration) * step
        ...
        self.solforce.append(np.copy(self.force))
    ...
    def compute_refl(self, step, size, z):
        ...
        if self.position[2] > z:
            ...
            if ((abs(x[0])) - (r) < projx):
                self.velocity[0] = np.sqrt(abs(velmod - self.velocity[1] ** 2 - self.velocity[2] ** 2))
            elif abs((size[0]) - (x[0])) - (r) < projx:
                self.velocity[0] = v[0]
                self.position[0] = self.position[0] - size[0]

            if abs((x[1])) - (r) < projy:
                self.velocity[1] = np.sqrt(abs(velmod - self.velocity[0] ** 2 - self.velocity[2] ** 2))
            elif abs((size[1]) - (x[1])) - (r) < projy:
                # self.heat_vel[1] = -1 * np.sqrt(abs(velmod - self.heat_vel[0] ** 2 - self.heat_vel[2] ** 2))
                self.velocity[1] = v[1]
                self.position[1] = self.position[1] - size[1]

            if abs((x[2])) - (r) < projz:
                self.velocity[2] = np.sqrt(abs(velmod - self.velocity[0] ** 2 - self.velocity[1] ** 2))
            elif abs((size[2]) - (x[2])) - (r) < projz:
                self.velocity[2] = -1 * np.sqrt(abs(velmod - self.velocity[0] ** 2 - self.velocity[1] ** 2))

            # a = np.sum(np.square(self.heat_vel))
            # if a != velmod:
            #     self.heat_vel *= np.sqrt(velmod/a)
            a = np.sum(np.square(self.velocity))
            if a != velmod:
                self.velocity *= np.sqrt(velmod/a)
...
def collide_particle(particle1, particle2, type, step):
    ...
    if type == 0:
        ...
        particle1.velocity = (-m2 / (m1 + m2)) * (vel) * 100
        particle2.velocity = (m1 / (m1 + m2)) * (vel) * 100

        # v1d = particle1.heat_vel + particle1.flow_vel
        # v2d = particle2.heat_vel + particle2.flow_vel
        v1d = particle1.velocity
        v2d = particle2.velocity
        ...
        particle1.velocity = v1d
        particle2.velocity = v2d
    ...
    #gas-surface
        ...
        particle1.velocity = (-m2 / (m1 + m2)) * (vel) * 100.
        particle2.velocity = np.array([0., 0., 0])
        imaginaryvel = (m1 / (m1 + m2)) * (vel) * 100.
...
def solve_step(particle_list, Ar_num, Al_num, step, size, center, R, Z, z1):
    ...
    for i in range (Ar_num + Al_num):
        particle_list[i].compute_step(step, forces[i], Z, size)